...
- 3889476: Make path remapping env-only: `editorPathMap` is no longer read from `tooltify.config.json`. The remap is derived solely from the `TOOLTIFY_HOST_ROOT` env (cwd → host root); any `editorPathMap` in the JSON is ignored. Without the env, no remap.

## 0.7.4

### Patch Changes

- 765c2cf: Derive `editorPathMap` from the `TOOLTIFY_HOST_ROOT` env when the server runs in a container, so consumers don't hardcode a per-machine host path. An explicit `editorPathMap` still takes precedence; without the env, paths are used as-is.
